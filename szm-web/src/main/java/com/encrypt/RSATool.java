...
public class RSATool {
	...
	public static void main(String[] args) {
		try {
			...
			String PUBLICKEY = Base64.encodeBase64String((byte[]) v[1]);
			String PRIVATEKEY = Base64.encodeBase64String((byte[]) v[0]);
			...
			String publicpwd = Base64.encodeBase64String(encryptByRSA((byte[]) v[1],"123-123".getBytes())) ;
			String privatepwd = Base64.encodeBase64String(encryptByRSA1((byte[]) v[0],"123-123".getBytes())) ;
			//使用私钥对密文进行解密 返回解密后的数据
			...
			//使用公钥对密文进行解密 返回解密后的数据
			byte[] publicDatas = decryptByRSA1(signPublic, Base64.decodeBase64(privatepwd.getBytes()));
			System.out.println("公钥解密1: " +new String(publicDatas, StandardCharsets.UTF_8));
