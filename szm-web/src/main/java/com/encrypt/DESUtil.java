...
public class DESUtil {
	...
	public static String encrypt(String content, String key) {
		try {
			...
			cipher.init(ENCRYPT_MODE, getSecretKey(key));// 初始化为加密模式的密码器
	...
	public static String decrypt(String content, String key) {
		...
		try {
			...
			//使用密钥初始化，设置为解密模式
			cipher.init(DECRYPT_MODE, getSecretKey(key));
...
//		String key = "sde@5f98H*^hsff%dfs$r344&df8543*er";
		String content = "222-3331";
		String key = "sde@5f98H*^hsff%dfs$r344&df8543*er";
		System.out.println("content:" + content);
		String s1 = DESUtil.encrypt(content, key);
		s1 = Base64.getEncoder().encodeToString(s1.getBytes("utf-8"));
		System.out.println("s1:" + s1);
		s1 = new String(Base64.getDecoder().decode(s1), StandardCharsets.UTF_8);
